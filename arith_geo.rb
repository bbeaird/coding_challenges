...
def arith_geo(arr)
  ...
  arr.each_with_index do |num, i|
    if !(arr[i + 1].nil?)
      common_differences << arr[i + 1] - arr[i]
      common_ratios << arr[i + 1] / arr[i]
    end
  end

  return "Arithmetic" if common_differences.uniq.length == 1
  return "Geometric" if common_ratios.uniq.length == 1
  return -1
end
